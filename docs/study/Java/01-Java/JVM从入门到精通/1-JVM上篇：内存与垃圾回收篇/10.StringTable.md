...
- 从JDK8 开始，设置 StringTable 长度的话，1009 是可以设置的最小值
...
![image-20200711093546398](./assets/10.StringTable/27b7bf706fc1724baf503eac9b49c7fc.png)

![image-20200711093558709](./assets/10.StringTable/c59830deeebca85d5b2e446211e4e28d.png)
...
![image-20210511111607132](./assets/10.StringTable/0ca01e17abb22f0fa16e87dd93b26a65.png)
...
public void test6(){
    ...
    final String s5 = "xi";
    String s6 = s4 + s5;
    System.out.println(s6 == s7); // true s4和s5是final修饰的，编译期就能确定s6的值了，s6指向的是常量池中的“shanxi”
}
```
...
/*
详情：1.StringBuilder的append方式，自始至终只创建过一个StringBuilder对象；
	使用String的字符串拼接方式：创建过多的StringBuilder和String对象
	2.使用String的字符串拼接方式：内存中由于创建了较多的StringBuilder和String对象，内存占用更大。
	如果进行GC，需要花费额外的时间
	
改进空间：在实际开发中，如果基本确定前前后后要添加的字符串长度不高于某个限定值highLevel的情况下，
	建议使用构造器 StrignBuilder s = new StringBuilder(highLevel); //new char[highLevel];
*/
public class Test {
    ...
    public static void testString(int times) {
        ...
        for (int i = 0; i < times; i++) {
            str += "test"; //每次循环都会创建一个StringBuilder和String
        }
    }
...
![image-20210511145542579](./assets/10.StringTable/b3657b493efaf41f83b72e2debffb14b.png)
 ...
 *    如果字符串常量池中没有 "1"，则会创建两个对象：
 *       堆空间中一个new对象
 *       字符串常量池中一个字符串常量"1"（注意：此时字符串常量池中已有"1"）
 *	  如果字符串常量池中已经有 "1"，则不会再创建常量池中的对象，而是直接引用已存在的 "1"。
 ...
 *    如果直接写String s3 = new String（"11"）,那么常量池中也会创建一个“11”，和jdk6一样
 *
 * ② s3.intern()
 *    由于此时常量池中并无"11"，为了节省内存，虚拟机不在常量池中创建“11”对象,而是把s3中记录的对象的地址存入常量池
 *    所以s3 和 s4 指向的都是一个地址
 */
String s3 = new String("1") + new String("1");
s3.intern();
...
![image-20210511152240683](./assets/10.StringTable/4c11070481d7c3cdb566163802cf582b.png)

![image-20200711145925091](./assets/10.StringTable/3a3bab69ad3c6302ea00c301dffb5193.png)
...
![image-20200711150859709](./assets/10.StringTable/bb9abdd927bd9ac80c1c18359d299629.png)

![image-20200711151326909](./assets/10.StringTable/e6c4796fa8d9b5dda9438c799bb45540.png)

**练习 2**

![image-20200711151433277](./assets/10.StringTable/ad465da04603bb228d6dde8950ee95ec.png)
...
目前，许多大规模的 Java 应用程序在内存上遇到了瓶颈。测量表明，在这些类型的应用程序中，大约 25%的 Java 堆实时数据集被`String'对象所消耗。此外，这些 "String "对象中大约有一半是重复的，其中重复意味着 "string1.equals(string2) "是真的。在堆上有重复的`String'对象，从本质上讲，这是一种内存的浪费。这个项目将在 G1 垃圾收集器中实现自动和持续的`String'重复数据删除，以避免浪费内存，减少内存占用。
