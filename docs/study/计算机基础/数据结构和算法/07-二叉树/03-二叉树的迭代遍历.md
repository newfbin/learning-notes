...
![二叉树前序遍历（迭代法）](./assets/03-二叉树的迭代遍历/二叉树前序遍历（迭代法）.gif)
...
![二叉树中序遍历（迭代法）](./assets/03-二叉树的迭代遍历/二叉树中序遍历（迭代法）.gif)
...
class Solution {
    public List<Integer> inorderTraversal(TreeNode root) {
        ...
        while (cur != null || !stack.isEmpty()) {
            if (cur != null) {     // 访问到最底层左节点
                stack.push(cur);   // 经过的节点入栈
                cur = cur.left;    // 左
            } else {
                cur = stack.pop();     // 弹出待处理节点
...
![前序到后序](./assets/03-二叉树的迭代遍历/20200808200338924.png)
...
class Solution {
    public List<Integer> postorderTraversal(TreeNode root) {
        ...
        while (!stack.isEmpty()) {
            TreeNode node = stack.pop();
            result.add(node.val);          // 中
            if (node.left != null) stack.push(node.left);  // 左先入栈（后续会先弹出右）
            if (node.right != null) stack.push(node.right); // 右
        }
        Collections.reverse(result); // 反转结果
        return result;
    }
}
```
