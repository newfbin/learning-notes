> 原文地址：https://javadoop.com/post/spring-aop-intro
...
![3](./assets/AOP-SpringAOP/7.png)

![3](./assets/AOP-SpringAOP/8.png)
...
![4](./assets/AOP-SpringAOP/4.png)

上面的两个 Advice 分别用于方法调用前输出参数和方法调用后输出结果。

现在可以开始配置了，我们配置一个名为 **spring_1_2.xml** 的文件：

![5](./assets/AOP-SpringAOP/5.png)

接下来，我们跑起来看看：

![6](./assets/AOP-SpringAOP/6.png)
...
#### autoproxy.BeanNameAutoProxyCreator -- 根据正则代理多个bean
...
![10](./assets/AOP-SpringAOP/10.png)
...
#### autoproxy.DefaultAdvisorAutoProxyCreator -- 根据正则匹配代理多个方法
...
![11](./assets/AOP-SpringAOP/11.png)

然后我们运行一下：

![12](./assets/AOP-SpringAOP/12.png)
...
- within：是 Spring AOP 中用于定义切点的一个表达式。它用于指定需要拦截的类范围。（Spring AOP 独有）

  > 如 @Pointcut("within(com.javadoop.dao..*)")
  >
  > - **`com.javadoop.dao`**：表示需要匹配的包路径。
  > - **`..`**：表示匹配该包及其所有子包。
  > - **`*`**：表示匹配包或子包中的所有类。
  >
  > 换句话说，`within(com.javadoop.dao..*)` 的作用是拦截位于 `com.javadoop.dao` 及其子包下的所有类中的方法。

- @annotation：方法上具有特定的注解，如 @Subscribe 用于订阅特定的事件。

  > 如 @Pointcut("execution(* *.*(..)) && @annotation(com.javadoop.annotation.Subscribe)")，表示拦截所有被 `@com.javadoop.annotation.Subscribe` 注解标注的方法，并且这些方法可以有任意返回值、任意方法名和任意参数列表。
  >
  > 如 @Around("@annotation(authCheck)")，该切点表达式表示切点匹配所有被 `@authCheck` 注解标注的方法。
...
![13](./assets/AOP-SpringAOP/13.png)

xml 的配置非常简单：

![14](./assets/AOP-SpringAOP/14.png)

> 这里是示例，所以 bean 的配置还是使用了 xml 的配置方式。

测试一下：

![15](./assets/AOP-SpringAOP/15.png)
...
![16](./assets/AOP-SpringAOP/16.png)
