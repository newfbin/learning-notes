...
1. 初始化 RabbitMQ 相关配置（InitRabbitMq 和 InitRabbitMqBean 类）
   - `InitRabbitMq` 类是一个普通的 Java 类，其中的 `doInit` 方法用于初始化 RabbitMQ 的交换机和队列。在方法中，首先创建一个 `ConnectionFactory` 实例并设置主机地址为 `localhost`，然后通过该工厂创建一个 `Connection` 和 `Channel`。接着声明一个名为 `code_exchange` 的 Direct 类型交换机，再创建一个名为 `code_queue` 的队列，并将该队列绑定到交换机上，绑定的路由键为 `my_routingKey`。`main` 方法调用 `doInit` 方法来执行初始化操作，不过这种方式需要手动运行 `main` 方法来初始化。
   - `InitRabbitMqBean` 类是一个 Spring 组件，通过 `@Value` 注解从配置文件中获取 RabbitMQ 的主机地址（如果配置文件中没有则使用默认值 `localhost`）。`@PostConstruct` 注解使得 `init` 方法在该 Bean 初始化完成后自动执行，其内部逻辑与 `InitRabbitMq` 类的 `doInit` 方法相同，也是声明交换机、创建队列并进行绑定。在实际的 Spring 应用中，这种方式会在应用启动时自动执行 RabbitMQ 的初始化配置。
2. 用户提交题目消息的发送（MyMessageProducer 类）
   - `MyMessageProducer` 类是一个消息生产者组件，通过 `@Resource` 注解注入了 `RabbitTemplate`。`RabbitTemplate` 是 Spring AMQP 提供的用于发送消息的工具类。
   - `sendMessage` 方法接收三个参数：`exchange`（交换机名称）、`routingKey`（路由键）和 `message`（要发送的消息内容）。在方法内部，通过 `rabbitTemplate.convertAndSend` 方法将消息发送到指定的交换机，并根据路由键进行消息的路由。当用户提交题目时，相关的业务逻辑会调用这个方法，将用户提交题目的相关信息（例如题目提交 ID，在代码中表现为一个字符串形式的 `questionSubmitId`）作为消息发送到 `code_exchange` 交换机，使用 `my_routingKey` 作为路由键。
3. 消息的接收和处理（MyMessageConsumer 类）
   - `MyMessageConsumer` 类是一个消息消费者组件，通过 `@Resource` 注解注入了 `JudgeService`，这个服务类负责具体的判题和更新提交状态的逻辑。
   - `@RabbitListener` 注解用于声明该方法是一个消息监听器，监听名为 `code_queue` 的队列，并且设置确认模式为 `MANUAL`（手动确认）。当有消息到达 `code_queue` 队列时，`receiveMessage` 方法会被触发执行。
   - 在 `receiveMessage` 方法中，首先记录接收到的消息内容，然后将接收到的字符串类型的 `message` 解析为 `long` 类型的 `questionSubmitId`。接着尝试调用 `judgeService.doJudge(questionSubmitId)` 方法进行判题操作。如果判题成功，则通过 `channel.basicAck(deliveryTag, false)` 方法手动确认消息已成功处理，RabbitMQ 会从队列中移除该消息；如果判题过程中出现异常，则通过 `channel.basicNack(deliveryTag, false, false)` 方法拒绝消息，消息会保留在队列中等待后续处理（这里的 `false` 表示不重新入队，即不会将消息放回队列头部）。
4. 整体流程总结
   - 当用户提交题目时，系统调用 `MyMessageProducer.sendMessage` 方法，将包含题目提交 ID 的消息发送到 `code_exchange` 交换机，根据 `my_routingKey` 路由键将消息路由到 `code_queue` 队列。
   - `MyMessageConsumer` 类监听 `code_queue` 队列，当消息到达时，`receiveMessage` 方法被触发，从消息中获取题目提交 ID，调用 `JudgeService` 的 `doJudge` 方法进行判题并更新提交状态。
   - 根据判题结果，手动确认或拒绝消息，以确保消息的可靠处理。
