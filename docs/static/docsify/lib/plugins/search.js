(function () {
  ...
  var INDEXS = {};
  // 保留原key定义，用于IndexedDB的存储键名，无需修改
  var LOCAL_STORAGE = {
  ...
  // ======================== 【改造核心1：新增 IndexedDB 工具方法 (开始)】 ========================
  // 数据库配置
  const DB_CONFIG = {
    name: 'DocsifySearchDB', // 数据库名
    version: 1,              // 版本号，更新结构时自增
    storeName: 'searchStore' // 对象仓库名，相当于表名
  };
  // 打开/创建数据库，返回数据库实例
  function openDB() {
  ...
  // IndexedDB 写入数据：key=存储键名，value=任意数据（支持对象，无需序列化）
  function setDBItem(key, value) {
    return new Promise(async (resolve, reject) => {
      const db = await openDB();
      const tx = db.transaction(DB_CONFIG.storeName, 'readwrite'); // 读写事务
      const store = tx.objectStore(DB_CONFIG.storeName);
      store.put({ key, value }); // put=有则更新，无则新增
      tx.oncomplete = () => { db.close(); resolve(true); };
      tx.onerror = (e) => { db.close(); reject(e.target.error); };
    });
  }
  // IndexedDB 读取数据：根据key获取对应值
  function getDBItem(key) {
    return new Promise(async (resolve, reject) => {
      const db = await openDB();
      const tx = db.transaction(DB_CONFIG.storeName, 'readonly'); // 只读事务
      const store = tx.objectStore(DB_CONFIG.storeName);
  ...
  // ======================== 【改造核心1：新增 IndexedDB 工具方法 (结束)】 ========================

  // ======================== 【改造核心2：替换原 saveData 方法 (原localStorage写入)】 ========================
  async function saveData(maxAge, expireKey, indexKey) {
    await setDBItem(expireKey, Date.now() + maxAge); // 写入过期时间，无需转字符串
    await setDBItem(indexKey, INDEXS);               // 写入索引对象，原生对象，无需JSON序列化！！！
  }
  ...
  // ======================== 【改造核心3：修改 init 方法 (原localStorage读取，异步化改造)】 ========================
  async function init(config, vm) {
    ...
    // 读取过期时间：替换 localStorage.getItem → getDBItem
    const expireTime = await getDBItem(expireKey);
    var isExpired = expireTime < Date.now();

    // 读取索引数据：替换 localStorage.getItem → getDBItem，无需JSON.parse！！！
    ...
    for (const path of paths) {
      ...
      try {
        const result = await Docsify.get(vm.router.getFile(path), false, vm.config.requestHeaders);
        INDEXS[path] = genIndex(path, result, vm.router, config.depth);
      } catch (err) {
        console.warn(`[Docsify搜索] 爬取文档失败: ${path}`, err);
        INDEXS[path] = {}; // 赋值空对象，避免重复爬取
      }
    }
